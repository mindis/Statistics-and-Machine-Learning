...
PRICE.vec <- as.numeric(t(DISC))
PRICE.vec <- as.numeric(t(DISP))
PRICE.vec <- as.numeric(t(SCALE))

#切片の設定
BP <- matrix(0, nrow=hhpt*choise, ncol=choise)
for(i in 1:hhpt){
...
for(i in 1:hh){
  r <- ((i-1)*(choise*pt)+1):((i-1)*(choise*pt)+choise*pt)
  SCALE.vec <- S
}
