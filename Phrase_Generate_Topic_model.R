...
#nグラムを生成
n_gram1 <- rep(0, f)
par <- c(0.25, 0.15, 0.075, 0.05, rep(0.025, 100))
for(i in 1:d){
  for(j in 1:w[i]){
    index <- doc_list[[i]][j]
    if(j==1){ 
...
for(i in 1:d){
  ...
  for(j in 1:w[i]){
    ...
    index <- doc_list[[i]][j]
    index_len <- index:doc_list[[i]][1]
    index_vec <- cumsum(n_gram1[index_len])==1:length(index_len)
    no0 <- c()
    
    for(l in 1:length(index_vec)){
      if(index_vec[l]==TRUE){
        no0 <- c(no0, 1)  
      } else {
        break
...
phi <- phit <- extraDistr::rdirichlet(k, alpha11) + 10^-100
psi <- psit <- extraDistr::rdirichlet(k, alpha12) + 10^-100
beta <- betat <- rbeta(k, 35.0, 15.0)
...
for(i in 1:d){
  ...
  for(j in 1:freq){
    if(j==1){
      #文章の先頭のトピックを生成
      z2 <- rmnom(1, 1, theta[index_d, ])
      z2_vec[j] <- as.numeric(z2 %*% 1:k)
      z1_vec[j] <- 0
    
    } else {
      
      #j-1とjの単語がフレーズかどうかを生成
      if(sum(phrase)==0){
        phrase <- c()
        z1_vec[j] <- rbinom(1, 1, pr[1])
        phrase <- c(phrase, z1_vec[j])
      } else {
        z1_vec[j] <- rbinom(1, 1, pr[sum(phrase)])
        phrase <- c(phrase, z1_vec[j])
      }
      
      if(j==1) next   #j=1の場合は次の単語へ 
      
      #フレーズに応じて2単語目以降のトピックを生成
      if(z1_vec[j]==1){
        #フレーズが続いているのならj-1と同じトピックを生成
        z2_vec[j] <- z2_vec[j-1]
