...
for(i in 1:seg){
  for(j in 1:k){
    tau1 <- runif(1, 1.5, 5.0)
    tau2 <- runif(1, 2.5, 10.0)
...
#セグメントごとのパラメータ
oldtheta <- matrix(0, nrow=seg0, ncol=k)
for(j in 1:seg0){
  index <- which(as.numeric(z_vec)==j)
  oldtheta[j, ] <- colMeans(Data[index, ])
}
...
for(rp in 1:R){
  ...
  LLind0 <- matrix(0, nrow=hhpt, ncol=ncol(z))
  ...
  for(j in 1:pt){
    ...
    r0[[j]] <- cbind(matrix(colSums(z[index, ]), nrow=length(index), ncol=ncol(z), byrow=T) - z[index, ], alpha/pt)
    r[index, ] <- r0[[j]] / (length(index)-1-alpha/pt)
  }
