...
##説明変数の発生
k <- 150   #説明変数数
freq <- rpois(hh, 100)   #ポアソン分布から頻度を発生
...
for(i in 1:1000){
  ...
  ##多項ロジットモデルのパラメータの設定
  b00 <- matrix(rnorm((k+1)*(select-1)*seg, 0, 0.35), nrow=k+1, ncol=(select-1)*seg)
  b01 <- ifelse(abs(b00) > 0.1, b0, 0)
  ...
  a00 <- matrix(rnorm((k+1)*seg, 0, 0.25), nrow=k+1, seg)
  a01 <- ifelse(abs(a00) > 0.1, b0, 0)
  a01[1, ] <- c(-1.1, 0.8)
  ...
  if((mean(Pr22)-mean(Pr21)) > 0.3 & mean(Pr22) > 0.55 & mean(Pr21) > 0.2) break
}
