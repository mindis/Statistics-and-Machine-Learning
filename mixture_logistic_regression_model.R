...
##説明変数の設定
X1 <- matrix(runif(N*(col-4), -1, 1), N, (col-4))
...
##回帰係数の発生
b1 <- c(runif(ncol(X1), 0, 1.0), runif(ncol(X2), -1.5, 1.5))   #回帰係数1
b2 <- c(runif(ncol(X1), 0, 1.9), runif(ncol(X2), -1.5, 1.5))   #回帰係数1
b3 <- c(runif(ncol(X1), 0, 2.3), runif(ncol(X2), -1.5, 1.5))   #回帰係数1
b4 <- c(runif(ncol(X1), 0, 2.5), runif(ncol(X2), -0.5, 1.5))   #回帰係数1
b1
b01 <- -0.3   #切片1
b02 <- 0.8   #切片2
b03 <- 0.4   #切片3
...
logit13 <- b03 + X %*% b3
logit14 <- b04 + X %*% b4
p11 <- exp(logit11) / (1+exp(logit11))
p12 <- exp(logit12) / (1+exp(logit12))
p13 <- exp(logit13) / (1+exp(logit13))
p14 <- exp(logit14) / (1+exp(logit14))

round(data.frame(y, p11, p12, p13, p14), 3)
...
#確率分布をプロット
hist(p1, breaks=20, col="#0000ff40", xlim=c(0, 1.0), ylim=c(0, 320), border = "#0000ff",
     xlab="rate", main="ディクレリ多項分布")
hist(p2, breaks=20, xlim=c(0, 1), ylim=c(0, 320), col="#ff00ff40", border = "#ff00ff",
     xlab="rate", main="ディクレリ多項分布", add=T)
hist(p3, breaks=20, xlim=c(0, 1), ylim=c(0, 320), col="#a5f0ff40", border = "#ff00ff",
     xlab="rate", main="ディクレリ多項分布", add=T)
hist(p4, breaks=20, xlim=c(0, 1), ylim=c(0, 320), col="#5f90f055", border = "#ff00ff",
     xlab="rate", main="ディクレリ多項分布", add=T)
...
betaf1 <- rep(coef.f[2:length(fc)], 4) + runif(k*col, -0.5, 0.5)
