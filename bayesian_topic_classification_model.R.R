...
#多項ロジットモデルの事前分布
zeta <- rep(0, (2*k+1)*(select-1))
tau0 <- diag(0.01, (2*k+1)*(select-1))
...
for(rp in 1:R){
  ...
  #新しいパラメータをサンプリング
  betad <- matrix(oldbeta, nrow=2*k+1, ncol=select-1)
  betan <- betad + mvrnorm(2*k+1, rep(0, select-1), diag(0.00025, select-1))
  
  #対数尤度と対数事前分布を計算
  lognew <- fr(betan, y_train, Data1, n1, select)$LL
  logold <- fr(betad, y_train, Data1, n1, select)$LL
  logpnew <- lndMvn(as.numeric(betan), zeta, tau0)
  logpold <- lndMvn(as.numeric(betad), zeta, tau0)
  ...
  if(rp%%keep==0){
    ...
    #予測分布を計算
    logit <- cbind(Data2 %*% matrix(oldbeta, nrow=2*k+1, ncol=select-1), 0)
...
round(cbind(matrix(oldbeta, nrow=2*k+1, ncol=select-1), b0), 3)
round(Pr, 3)
