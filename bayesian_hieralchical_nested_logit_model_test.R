...
##個体間パラメータの設定
#個体間分散共分散行列を設定
Cov1 <- corrM(col=k1-1, lower=-0.55, upper=0.9, eigen_lower=0.025, eigen_upper=0.35)   #相関行列を作成
...
theta11 <- matrix(c(runif(2*k2, -0.4, 0.5), runif((member-1)*k2, -0.4, 0.55), runif((c_num-1)*k2, -0.4, 0.5), 
                    ...
                    runif(k2, -0.4, 0.4), runif(k2, -0.35, 0.35)), nrow=k2, ncol=k1-1, byrow=T)

#ログサム変数のパラメータの設定
theta20 <- runif(1, -0.2, 0.5)
theta21 <- runif(k2, -0.3, 0.4)
...
##個体内回帰パラメータの設定
beta <- cbind(1, ZM) %*% theta1 + mvrnorm(hh, rep(0, k1-1), Cov1/3)   #回帰係数のパラメータ
rho.par <- cbind(1, ZM) %*% theta2 + rnorm(hh, 0, sqrt(1/3))   #ログサム変数のパラメータ
...
for(i in 1:hh){
  ...
  logit1.list[[i]] <- cbind(1, XM1[ID$id==i, ]) %*% beta1[i, ] + 
                                  logsum.list[[i]]*rowSums(CARD[ID$id==i, ]) * rho[i, ]
...
loglike <- function(x, y1, y2, XM1, XM2, CARD, type, member, c_num, hhpt){
  ...
  #パラメータの設定
  beta1 <- x[c(1, 3:(length(x)-3))]
  beta2 <- x[c(2:(member-1 + c_num-1 + 2), length(x)-2, length(x)-1)]
  rho <- exp(x[length(x)])/(1+exp(x[length(x)]))   #パラメータは0～1
  ...
  #ロジットの定義
  logit1 <- cbind(1, XM1) %*% beta1 + logsum*rowSums(CARD) * rho    #カード所有有無のロジット
...
for(i in 1:hh){
  ...
  CARD_ind[[i]] <- rowSums(CARD[index.x[i, ], ])
...
##事前分布の設定
Deltabar1 <- matrix(0, nrow=ncol(cbind(1, ZM)), ncol=k1)   #回帰パラメータの階層モデルの回帰係数の平均の事前分布
...
nu1 <- (ncol(ZM)+1)+k1   #逆ウィシャート分布の自由度
V1 <- nu1 * diag(k1)   #逆ウィシャート分布のパラメータ
...
##初期値の設定
#回帰ペラメータの初期値
tau1 <- mvrnorm(hh, rep(0, k1-1), diag(0.3, k1-1))
oldbetas <- matrix(x1[1:(k1-1)], nrow=hh, ncol=k1-1, byrow=T) + tau1
...
#事前分布の設定
oldDelta1 <- ginv(t(ZMi) %*% ZMi) %*% t(ZMi) %*% cbind(oldbetas, oldrho)
oldVbeta1 <- 1/hh * (t(oldbetas - ZMi %*% oldDelta1) %*% (cbind(oldbetas, oldrho) - ZMi %*% oldDelta1))
...
index.par2 <- c(2:(member-1 + c_num-1 + 2), ncol(XM2_ind[[1]])+3, ncol(XM2_ind[[1]])+4)
...
LL_nest <- function(beta, rho, y1, y2, XM1, XM2, CARD, type, member, c_num, n, index.par1, index.par2, index.logsum){
  ...
  #ロジットの定義
  logit1 <- XM1 %*% beta1 + logsum*CARD * rho1    #カード所有有無のロジット
...
for(rp in 1:R){
  ...
  for(i in 1:hh){
    #回帰パラメータの対数尤度と対数事前分布
    lognew1[i, ] <- LL_nest(beta=betan[i, ], rho=rhon[i, ], y1=y1_ind[[i]], y2=y2_ind[[i]], XM1=XM1_ind[[i]], XM2=XM2_ind[[i]], 
                       CARD=CARD_ind[[i]], type=type, member=member, c_num=c_num, n=n, index.par1=index.par1, 
                       index.par2=index.par2, index.logsum=index.logsum)
    logold1[i, ] <- LL_nest(beta=betad[i, ], rho=rhod[i, ], y1=y1_ind[[i]], y2=y2_ind[[i]], XM1=XM1_ind[[i]], XM2=XM2_ind[[i]], 
                       CARD=CARD_ind[[i]], type=type, member=member, c_num=c_num, n=n, index.par1=index.par1, 
                       index.par2=index.par2, index.logsum=index.logsum)
  ...
  index.adopt1 <- subset(1:hh, as.numeric(alpha1) > rand1)
  oldbetas.r[index.adopt1, ] <- betan[index.adopt1, ]
  index.adopt2 <- subset(1:hh, as.numeric(alpha2) > rand2)
